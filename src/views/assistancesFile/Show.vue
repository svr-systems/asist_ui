<template>
  <v-card :loading="isLoading">
    <v-card-title>
      <v-row dense>
        <v-col cols="10">
          <BtnBack :route="{ name: `${routeName}/file` }" />
        ...
        <v-col v-if="item" cols="2" class="text-right">
          <v-btn
            ...
            :to="{
              name: 'assistance',
              params: { file_id: getEncodeId(itemId) },
            }"
          >
            <v-icon>mdi-account-multiple-check</v-icon>
            <v-tooltip activator="parent" location="left">Asistencias</v-tooltip>
          </v-btn>
        </v-col>
      </v-row>
    </v-card-title>
        ...
        <v-col cols="12">
          <v-card>
            ...
            <v-card-text>
              <v-row dense>
                <v-col cols="12" md="3">
                  <VisVal label="Archivo" :value="item.assistance_file" />
                </v-col>
                <v-col cols="12" md="3">
                  <VisVal label="ID" :value="item.uiid" />
                </v-col>
              </v-row>
            </v-card-text>
          </v-card>
